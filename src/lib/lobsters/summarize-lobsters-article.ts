...
품질: **배경 설명**과 **향후 전망**을 포함해 서술형 깊이를 더한다. 필드 문자 합계가 **최소 600자**(공백 포함) 이상 되도록 분량을 확보한다.

아래 JSON **한 개만** 출력하라. 마크다운 코드펜스·설명 문장 밖의 텍스트 금지.

스키마:
- "postTitle": 문자열. 반드시 "[Lobsters 요약]" 으로 시작한 뒤, 원문을 한 줄로 잘 요약한 제목을 붙여라.
- "introduction": 문자열. 2~5문장 도입.
- "backgroundContext": 문자열. **배경 설명** — 산업·기술 맥락, 왜 주목되는지. "\\n\\n" 문단 구분 허용.
- "sections": 배열. 원문에 맞게 **최소 3개, 많으면 6개까지** 객체. 각 객체는 "title"과 "content" 필수.
- "valueAndInsight": 문자열. 가치·시사점·실무 영향.
    ...
    for (const apiVersion of GEMINI_API_VERSION_CHAIN) {
      try {
        ...
        if (!parsed.ok) {
          ...
          console.warn('[lobsters/summarize] JSON 파싱 실패 → 다음 후보', { modelId, apiVersion });
          continue;
        }
        const data = parseGeekNewsArticleJson(parsed.value);
        if (!data) {
          lastErr = new Error('Invalid article JSON shape');
          console.warn('[lobsters/summarize] 스키마 불일치 → 다음 후보', { modelId, apiVersion });
          continue;
        }
        console.log('[lobsters/summarize] Gemini 요약 완료', {
          modelId,
          apiVersion,
        ...
        if (isGeminiModelNotFoundForFallback(e)) {
          console.warn('[lobsters/summarize] 모델·API 경로 불가 → 다음 후보', {
            modelId,
            apiVersion,
        ...
        if (classified.category === 'SERVER') {
          console.warn('[lobsters/summarize] 일시 과부하(503 등) → 다음 모델·버전 시도', {
            modelId,
            apiVersion,
          });
          continue;
        }
        console.warn('[lobsters/summarize] 일시 오류 → 다음 후보', {
          modelId,
          apiVersion,
