...
- "easySummary": 문자열. 핵심을 중학생 눈높이로 설명한다. 문단 구분은 "\\n\\n"(줄바꿈 두 번)을 사용한다.
- "futureImpact": 문자열. **이 기술·연구가 바꿀 미래**에 대해 한국어로 서술한다(2~6문장).
...
function parseMitNewsSummaryJson(value: unknown): MitNewsSummaryJson | null {
  ...
  if (easySummary.length < 60 || futureImpact.length < 30) return null;
  return { postTitle, easySummary, futureImpact };
}
