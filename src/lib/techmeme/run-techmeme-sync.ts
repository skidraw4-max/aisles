...
export async function runTechmemeSync(options: { force: boolean }): Promise<TechmemeSyncResult> {
  ...
  for (const story of aiRanked) {
    ...
    if (geminiOrdinal >= MAX_GEMINI_CALLS_PER_SYNC_RUN) {
      console.warn('[techmeme] Gemini 호출 상한 도달 — 나머지 항목 스킵', {
    ...
    if (!sum.ok) {
      ...
      if (isGeminiRateLimitMessage(sum.error)) {
        console.warn('[techmeme] Gemini rate limit 지속 — 후속 기사 스킵');
        break;
      }
      continue;
    }
    ...
    if (shouldSkipThinLoungePost(content, { source: 'techmeme', title, externalUrl })) {
      results.push({
        externalUrl,
