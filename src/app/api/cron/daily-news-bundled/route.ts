/**
 * The Verge → GeekNews → Hacker News → Lobsters → Techmeme → AI Breakfast → MIT News 순차 실행 (한 소스 실패 시 다음 소스 계속)
 *
 * - 환경: `CRON_SECRET`(필수)
  ...
  return NextResponse.json({
    ...
    hackernews:
      'thrown' in result.hackernews
        ...
        : result.hackernews,
    lobsters:
      'thrown' in result.lobsters ? { thrown: serializeThrown(result.lobsters.thrown) } : result.lobsters,
    techmeme:
      'thrown' in result.techmeme ? { thrown: serializeThrown(result.techmeme.thrown) } : result.techmeme,
    aibreakfast:
