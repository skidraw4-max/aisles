...
export function UploadForm({ editInitial = null }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!isLounge && description.trim().length < MIN_POST_DESCRIPTION_LENGTH) {
      setFormError(
        `설명(Description)은 ${MIN_POST_DESCRIPTION_LENGTH}자 이상 입력해 주세요. (LOUNGE 복도는 제외)`
      );
      return;
    }
        ...
        <label className={styles.label}>
          ...
          {isLounge ? (
            ...
            <span className={styles.optionalMark}>
              {' '}
              · 최소 {MIN_POST_DESCRIPTION_LENGTH}자 (검색·애드센스 품질)
            </span>
          )}
          <textarea
            ...
            placeholder={
              isLounge
                ...
                : `프롬프트 팁·생성 의도 등을 ${MIN_POST_DESCRIPTION_LENGTH}자 이상 적어 주세요. 이미지 붙여넣기 지원.`
            }
            rows={isLounge ? 6 : 4}
          />
